...
项目以英文作为未特殊标注内容的第一语言，包括代码注释、文档、元数据和默认界面文案。
`README.md` 默认显示英文，本文件为中文版本，可通过两份文件顶部的链接切换。
明确标注的中文翻译、语言测试、专有名称、已有中文城市数据键及用户自行填写的照片说明保留原文。
新增界面文案统一放入 `src/i18n.js`，先提供英文，再补充 `zh` 翻译；更新文档时同步维护两种版本。
...
「添加照片 / Add Photos」支持电脑和手机选图，按地点和城市分别保存到此浏览器的 IndexedDB。再次添加会追加，不覆盖原有照片，刷新后仍保留。打开自己添加的照片可移除，不会删除原始文件。
照片不会上传、写入仓库或跨设备同步。清除网站数据、无痕模式或浏览器回收存储可能导致照片丢失，请保留原图。localhost 与正式站点的存储相互独立；IndexedDB 按域名来源隔离，不是同域名不同项目之间的身份认证边界。
保存前自动将长边缩至最多 1920px，并生成 480px 缩略图。支持 JPEG、PNG、WebP、AVIF，每张原文件不超过 25MB，每个城市最多 10 张（含网站发布的照片）。HEIC/HEIF 依赖浏览器解码能力，不支持时请导出为 JPEG。存储或解码失败会提示，不覆盖已有照片。
...
路径相对于 `src/`，构建后保持相同目录结构。访客在本机添加的照片不参与构建。
